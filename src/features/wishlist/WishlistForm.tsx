...
import { ChevronDown, ImageIcon, LinkIcon, Plus, Search, Trash2 } from "lucide-react";
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[300px_1fr]">
      ...
      <Card>
        ...
        <form id="wishlist-form" className="space-y-5" onSubmit={handleSubmit}>
          {/* 基本入力 */}
          <div>
          ...
          <div>
            ...
            <textarea id="actualPriceMemo" className="mt-2 min-h-24 w-full rounded-md border border-line px-3 py-2" value={actualPriceMemo} onChange={(event) => setActualPriceMemo(event.target.value)} placeholder="ポイントやクーポンを含めた実質価格の見立てや確認日をメモできます。" />
          </div>

          <div>
            <label className="text-sm font-semibold" htmlFor="targetSale">関連セール</label>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted">価格内訳・比較候補・参考リンク・補足メモを入力できます。</p>
          </div>
          {expanded ? (
          ...
          {/* 保存ボタン（スマホは画面下部に固定表示、PCはインライン） */}
          <div className="sticky bottom-16 z-30 -mx-5 -mb-5 mt-2 border-t border-line bg-white/95 px-5 py-3 backdrop-blur md:bottom-4 lg:static lg:mx-0 lg:mb-0 lg:border-0 lg:bg-transparent lg:px-0 lg:py-0 lg:backdrop-blur-none">
            <Button type="submit" disabled={saving} className="w-full lg:w-auto">{saving ? "保存中" : "保存する"}</Button>
          </div>
        </form>
